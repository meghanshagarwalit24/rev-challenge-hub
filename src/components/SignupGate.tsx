...
export function SignupGate({ onSuccess }: SignupGateProps) {
  ...
  const { signIn: googleSignIn } = useGoogleSignIn({
    onSuccess: async (profile) => {
      if (!consent) { setErr("Please accept the consent to continue with Google"); return; }
      setErr("");
      setLoading(true);
      await completeSignup(profile.email, profile.name || name || "Google User", referredBy || undefined);
    },
    ...
    >
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {step === "contact" ? (
            <motion.form key="contact" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={sendOtp} className="mt-5 space-y-3">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1.5 w-full bg-background/60 border border-border rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-ring"
                />
                <p className="mt-1 text-[11px] text-muted-foreground">We'll send a one-time code to verify.</p>
              </div>
              <div>
                ...
                <input
                  ...
                  className="mt-1.5 w-full bg-background/60 border border-border rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-ring"
                />
                <p className="mt-1 text-[11px] text-muted-foreground">Enter your friend's contact who referred you — they'll get more chances to win! 🏆</p>
              </div>
            ...
            <motion.form key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={verify} className="mt-5 space-y-4">
              ...
              <button type="button" onClick={() => { setStep("contact"); setOtp(""); setErr(""); }} className="w-full text-sm text-muted-foreground hover:text-foreground transition-colors">
                ← Change number/email
              </button>
            </motion.form>
          )}
